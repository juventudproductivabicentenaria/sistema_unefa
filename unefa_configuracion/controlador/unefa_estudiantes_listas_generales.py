...
class listas_generales_estudiantes(http.Controller):
    ...
    @http.route(['/descargar/listas_general_estudiantes/<model("unefa.coordinacion"):coordinacion_data>'],
                type='http', auth='user', website=True)
    def descargar_lista_general_estudiante(self,coordinacion_data, **post):
       
        cr, uid, context = request.cr, request.uid, request.context
        registry = http.request.registry
        reportname='unefa_configuracion.lista_generales_estudiantes_qweb'
        carrera_id=coordinacion_data.carrera_id.id
        regimen=coordinacion_data.regimen
        ...
        hoy=str(date.today())
        j='hola'
        coordinacion=str(coordinacion_data.nombre)
